...
//notifies follower (userID == follower_id) about advertisement (title == advertisement_title) published by masseur (userID == masseur_id)
    ...
    schema.findOneAndUpdate(
      ...
      (err, updatedUser) => {
        if (err) {
          ...
          reject({succeeded: false, description: "error creating notification",  error: "error creating notification"})
        }
        resolve({ succeeded: true, newNotification: updatedUser.notifications[updatedUser.notifications.length-1], follower_id: follower_id })
      })
  })
}
...
exports.addAdvertisement = function(advertisement_title, advertisement_body, masseur_id){
  return new Promise((resolve, reject) => {
    ...
    Masseur.findOneAndUpdate({_id: masseur_id},
      ...
      (err, updatedUser) => {
        if (err) {
          reject({succeeded: false, error: err, description: "error handling adding advertisement"})
          return
        }

        console.log(updatedUser.followers.length)
        ...
        if (updatedUser.followers.length > 0){
          ...
          Promise
          .allSettled(notificationPromises)
          .then(results => {
            ...
            results
              ...
              resolve({succeeded: true, notifications: notificationInfoToRet})
            return
          }).catch(err => {
            reject({succeeded: false, error: err, description: "error handling notification promises"})
          })
        } else {
          resolve({succeeded: true, notifications: [] })
        }
      })
      .catch(err => {
        reject({succeeded: false, error: err, description: "error handling adding advertisement"})
